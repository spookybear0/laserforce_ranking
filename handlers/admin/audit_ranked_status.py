...
async def audit_ranked_status(request: Request, type: str) -> str:
    ...
    async def task():
        ...
        if type in ["sm5", "all"]:
            ...
            for game in sm5_games:
                logger.debug(f"Auditing SM5 Game ID {game.id} for ranked eligibility")
        ...
        if type in ["laserball", "all"]:
            ...
            for game in laserball_games:
                logger.debug(f"Auditing Laserball Game ID {game.id} for ranked eligibility")
    ...
    request.app.add_task(task(), name="Audit Ranked Status")
