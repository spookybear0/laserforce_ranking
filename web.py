...
async def log_sm5_game_post(r: web.Request):
    ...
    if not bcrypt.checkpw(
        ...
        b"$2b$10$d85KPH1vMLV20zt0E.0uxOO/Kb3zhgxLp7joElh/K.nnmI/jkTEG.",
    ):
        return web.Response(
            text="403: Access Denied! (you have been reported to the fbi)"
        )
    ...
    for red_player in range(1, 8):
        ...
        try:
            ...
            return web.Response(
                text=f"401: Error, invalid data! (codename, green, {player_name})"
            )
        player_id = player.player_id
        game = SM5GamePlayer(
            player_id, 0, Team.RED, Role(player_role), int(player_score)
        )
    ...
    for green_player in range(1, 8):
        ...
        try:
            ...
            return web.Response(
                text=f"401: Error, invalid data! (codename, red, {player_name})"
            )
        player_id = player.player_id
        game = SM5GamePlayer(
            player_id, 0, Team.GREEN, Role(player_role), int(player_score)
        )
    ...
    for player in players:
        if not player.game_player.role.value in [
            "scout",
            "heavy",
            "commander",
            "medic",
...
async def log_laserball_game_post(r: web.Request):
    await init_sql()
    data = await r.post()
    ...
    for red_player in range(1, 8):
        ...
        try:
            ...
            return web.Response(
                text=f"401: Error, invalid data! (codename, blue, {player_name})"
            )
        player_id = player.player_id
        game = SM5GamePlayer(
            player_id, 0, Team.RED, Role(player_role), int(player_score)
        ...
        player.game_player = game
        red_players.append(player)
    for blue_player in range(1, 8):
        try:
            player_name = data[f"gname{blue_player}"]
            player_role = data[f"grole{blue_player}"]
            player_score = data[f"gscore{blue_player}"]
            if player_name == "" or player_role == "" or player_score == "":
                break
        except KeyError:
            break
        ...
        player_id = player.player_id
        game = SM5GamePlayer(
            player_id, 0, Team.BLUE, Role(player_role), int(player_score)
        )
        blue_game_players.append(game)
        player.game_player = game
...
async def players_get(r: web.Request):
    ...
    return await render_template(
        r, "players.html", players=await get_top_100(15, 15 * page), page=page
    )
...
async def player_get(r: web.Request):
    ...
    return await render_template(r, "player.html", player=player, ordinal=ordinal)
