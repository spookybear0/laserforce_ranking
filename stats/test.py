...
class Player:
    ...
    async def _get_lifetime_stats(self):
        cursor = mydb.cursor()
        cursor.execute(
            "SELECT SUM(goals), SUM(assists), SUM(steals), SUM(clears), SUM(blocks) FROM laserball_game_players WHERE player_id = %s",
            (self.player_id,))
...
class Game:
    ...
    async def _get_game_players_team(self, team: Team):
        cursor = mydb.cursor()
        cursor.execute(f"SELECT * FROM {self.type.value}_game_players WHERE `game_id` = %s AND `team` = %s",
                       (self.id, team.value), )
        q = cursor.fetchall()
        final = []
...
async def main():
    ...
    bcr.bar_chart_race(df=df, filename="out.mp4", title="SM5 players at ILT skill rating over time", n_bars=20,
                       period_fmt="Game {int(x)}")
