...
class TestSm5Helper(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_sm5_notable_events(self):
        ...
        self.assertEqual([NotableEvent(seconds=100, event="Dr. Medic (Earth Medic) is eliminated"),
                          NotableEvent(seconds=150,
                                       event="Some Player got their nuke canceled (NO IDEA HOW)"),
                          NotableEvent(seconds=153, event="Some Player detonates a nuke"),
                          NotableEvent(seconds=200, event="AmmoBoi (Fire Ammo) is eliminated"),
                          NotableEvent(seconds=402,
                                       event="Dr. Medic had their nuke canceled by AmmoBoi"),
                          NotableEvent(seconds=506,
                                       event="Some Player had their nuke canceled by FRIENDLY FIRE (Dr. Medic)"),
                          NotableEvent(seconds=606,
                                       event="Dr. Medic had their nuke canceled by THEIR OWN RESUP (Some Player)"),
                          NotableEvent(seconds=701,
                                       event="AmmoBoi had their nuke canceled by Some Player's nuke - they nuked first"),
                          NotableEvent(seconds=705, event="Some Player detonates a nuke"),
                          NotableEvent(seconds=800,
                                       event="AmmoBoi had their nuke canceled by Some Player's nuke - and they nuked later"),
                          NotableEvent(seconds=805, event="Some Player detonates a nuke"),
                          ], notable_events)
