...
def calculate_laserball_mvp_points(player: LaserballGamePlayer):
    ...
    mvp_points += (
        player.clears * 0.25
    )  # clear implies a steal so the total gained is 0.75
...
async def get_top_100(amount: int = 100, start: int = 0):
    q = await sql.fetchall(
        f"SELECT codename, player_id FROM players ORDER BY LENGTH(player_id), player_id ASC LIMIT {amount} OFFSET {start}"
    )
    return list(q)
...
async def get_games_played(player_id: str):
    q = await sql.fetchone(
        "SELECT COUNT(*) FROM `sm5_game_players` WHERE `player_id` = %s", (player_id)
    )
    q2 = await sql.fetchone(
        "SELECT COUNT(*) FROM `laserball_game_players` WHERE `player_id` = %s",
        (player_id),
    )
    return q[0] + q2[0]
...
async def database_player(player_id: str, codename: str) -> None:
    ...
    if await sql.fetchone(
        "SELECT id FROM `players` WHERE `player_id` = %s", (player_id)
    ):  # update stuff.
        await sql.execute(
            """UPDATE `players` SET `codename` = %s WHERE `player_id` = %s""",
            (codename, player_id),
        )

        await sql.execute(
            """UPDATE `players` SET `ipl_id` = %s WHERE `player_id` = %s""",
            (ipl_id, player_id),
        )
    else:
        try:
            await sql.execute(
                """INSERT IGNORE INTO `players` (player_id, ipl_id, codename)
                                VALUES (%s, %s, %s)""",
                (player_id, ipl_id, codename),
            )
...
async def log_sm5_game(game: SM5_Game) -> None:
    raise DeprecationWarning("Logging sm5 games is deprecated")
    elo_logger.info("Inserting sm5 game")
    await sql.execute(
        """INSERT INTO `sm5_games` (winner)
                        VALUES (%s);""",
        (game.winner),
    )
    ...
    game.red, game.green = await update_elo(game.red, game.green, game.winner)
...
async def log_laserball_game(game: Laserball_Game) -> None:
    elo_logger.info("Inserting laserball game")
    await sql.execute(
        """INSERT INTO `laserball_games` (winner)
                        VALUES (%s);""",
        (game.winner),
    )
    elo_logger.debug("Inserted game")
...
async def fetch_game_players(game_id: int, type="sm5") -> List[SM5GamePlayer]:
    q = await sql.fetchall(
        f"SELECT * FROM `{type}_game_players` WHERE `game_id` = %s", (game_id)
    )
    final = []
...
async def fetch_game_players_team(
    game_id: int, team: Team, type="sm5"
) -> List[SM5GamePlayer]:
    q = await sql.fetchall(
        f"SELECT * FROM `{type}_game_players` WHERE `game_id` = %s AND `team` = %s",
        (game_id, team.value),
    )
    final = []
...
async def init_sql():
    global sql
    if not sql:
        sql = await mysql.MySQLPool.connect(
            config["db_host"],
            config["db_user"],
            config["db_password"],
            config["db_database"],
